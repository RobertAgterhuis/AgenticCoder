...
describe('azure-resource-graph MCP (stdio) integration', () => {
  it('spawns the real python server and supports ping + tools/list', async (t) => {
    ...
    const cwd = path.join(repoRoot, '.github', 'mcp', 'azure-resource-graph-mcp');

    // Skip cleanly if python isn't available or module can't be imported.
    const probe = await runProcess(python, ['-c', 'import azure_resource_graph_mcp; print("ok")'], {
      cwd,
      timeoutMs: 5000
    });
    ...
    if (probe.code !== 0) {
      t.skip(
        `Python resource graph MCP not available (code=${probe.code}). ` +
    ...
    const client = new McpStdioToolClient({
      name: 'azure-resource-graph-stdio-integration',
      command: python,
      args: ['-m', 'azure_resource_graph_mcp', 'mcp'],
      cwd
    });
    ...
    try {
      ...
      assert.deepStrictEqual(ping, { status: 'ok', service: 'azure-resource-graph-mcp' });
    } finally {
