...
describe('End-to-end workflow (offline)', () => {
  ...
  it('runs TaskExtraction -> ResourceAnalyzer -> CostEstimator without live servers', async () => {
    ...
    assert.strictEqual(result.outputs.resources.length, 4);
    ...
    assert.strictEqual(result.outputs.breakdown.length, 4);

    // Deterministic total: function (0.20) + storage (1.84) + logs (2.76) + kv (0.03) = 4.83
    assert.ok(Math.abs(result.outputs.totalCost - 4.83) < 0.01);
